...
def mid_weekday_peak_period_travel_times():
    ...
    for ea in range(0, len(sList)):
        ...
        detector_query = 'SELECT detectorid From `%s` WHERE stationid = "%s"' %(DETECTOR_DOMAIN, sList[ea])
        ...
        if (start % 7) in [4,5,6]:
            for det in dList:
                for hr in  ["07","08","16","17"]:
                    l_query = 'SELECT speed FROM `%s` WHERE detectorid = "%s" AND starttime like "%s-%s-%sT%s:%s" AND speed != "" AND speed != "0" ' %(LOOPDATA_DOMAIN, det, year, month, day, hr, per)
